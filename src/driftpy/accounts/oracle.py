...
async def get_oracle_price_data_and_slot(connection: AsyncClient, address: Pubkey, oracle_source=OracleSource.PYTH()) -> DataAndSlot[
    ...
    if 'Pyth' in str(oracle_source):
        ...
        return DataAndSlot(data=OraclePriceData(PRICE_PRECISION, 0, 1, 1, 0, True), slot=0)
    else:
...
async def _parse_pyth_price_info(resp: GetAccountInfoResp) -> (PythPriceInfo, int, int, int):
    ...
    derivations = dict(
        (type_, derivations[type_.value - 1]) for type_ in [EmaType.EMA_CONFIDENCE_VALUE, EmaType.EMA_PRICE_VALUE])
    ...
    return (PythPriceInfo.deserialise(buffer, offset, exponent=exponent), last_slot, twac, twap)
