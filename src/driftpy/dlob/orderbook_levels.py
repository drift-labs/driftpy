...
class L2OrderBook:
    def __init__(self, asks: List[L2Level], bids: List[L2Level], slot: Optional[int] = None):
        self.asks = asks
        self.bids = bids
        self.slot = slot
...
class L3OrderBook:
    def __init__(self, asks: List[L3Level], bids: List[L3Level], slot: Optional[int] = None):
        self.asks = asks
        self.bids = bids
        self.slot = slot
    ...
    for level in levels:
        ...
        if grouped_levels and grouped_levels[-1].price == price:
            ...
            for source, additional_size in level.sources.items():
                current_level.sources[source] = current_level.sources.get(source, 0) + additional_size
        else:
