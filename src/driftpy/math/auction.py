from driftpy.types import Order, is_one_of_variant, is_variant
...
def get_auction_price(order: Order, slot: int, oracle_price: int) -> int:
    if is_one_of_variant(order.order_type, ['Market', 'TriggerMarket', 'Limit', 'TriggerLimit']):
...
def get_auction_price_for_fixed_auction(order: Order, slot: int) -> int:
    ...
    if is_variant(order.direction, "Long"):
        price_delta = order.auction_end_price - order.auction_start_price \
...
def get_auction_price_for_oracle_offset_auction(order: Order, slot: int, oracle_price: int) -> int:
    ...
    if is_variant(order.direction, "Long"):
        price_offset_delta = order.auction_end_price - order.auction_start_price \
            * delta_numerator // delta_denominator
    else:
        price_offset_delta = order.auction_start_price - order.auction_end_price \
            * delta_numerator // delta_denominator
