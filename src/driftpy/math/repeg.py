from driftpy.math.amm import calculate_swap_output, calculate_amm_reserves_after_swap, get_swap_direction
...
#     print(market)
    ...
    net_user_position = MarketPosition(
            market_index, market.base_asset_amount, 0, 0, 0, 0, 0, 0, 0, 0, PublicKey(0), 0, 0
        )
    ...
    marketNewK.amm.sqrt_k = np.sqrt( marketNewK.amm.base_asset_reserve * marketNewK.amm.quote_asset_reserve)
...
def calculate_rebalance_market(market, market_index):
    ...
    marketNewPeg.amm.quote_asset_reserve = newSqrtK**2 / marketNewPeg.amm.base_asset_reserve
...
def calculate_repeg_cost(market, new_peg):
    ...
    cost2 = delta_quote_reserves* (market.amm.peg_multiplier - new_peg)/AMM_TIMES_PEG_TO_QUOTE_PRECISION_RATIO
    
    mark_delta = (market.amm.quote_asset_reserve/market.amm.base_asset_reserve) \
    * (new_peg - market.amm.peg_multiplier)/PEG_PRECISION
...
def calculate_freepeg_cost(market, market_index, target_price, bonus=0):
    ...
    # print('------')
    bonly3, market3 = calculate_curve_op_cost(market, market_index, pk*p, pk*1/p, new_peg)
