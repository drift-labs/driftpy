...
async def test_user_decode(program: Program):
    ...
    for index, user_account_buffer_string in enumerate(user_account_buffer_strings):
        ...
        (anchor_size, custom_size, anchor_time, custom_time) = user_account_decode(
            program, user_account_buffer, index
        )
...
def user_account_decode(program: Program, user_account_buffer: bytes, index: int):
    ...
    anchor_user_account: UserAccount = program.coder.accounts.decode(
        user_account_buffer
    )
    ...
    for anchor_s, custom_s in zip_generator(
        anchor_spot_generator, custom_spot_generator
    ):
    ...
    for anchor_p, custom_p in zip_generator(
        anchor_perp_generator, custom_perp_generator
    ):
    ...
    for anchor_o, custom_o in zip_generator(
        anchor_order_generator, custom_order_generator
    ):
        cmp_orders(anchor_o, custom_o)

    assert (
        anchor_user_account.last_add_perp_lp_shares_ts
        == custom_user_account.last_add_perp_lp_shares_ts
    )
    ...
    assert (
        anchor_user_account.total_social_loss == custom_user_account.total_social_loss
    )
    assert anchor_user_account.settled_perp_pnl == custom_user_account.settled_perp_pnl
    assert (
        anchor_user_account.cumulative_spot_fees
        == custom_user_account.cumulative_spot_fees
    )
    assert (
        anchor_user_account.cumulative_perp_funding
        == custom_user_account.cumulative_perp_funding
    )
    assert (
        anchor_user_account.liquidation_margin_freed
        == custom_user_account.liquidation_margin_freed
    )
    ...
    assert (
        anchor_user_account.next_liquidation_id
        == custom_user_account.next_liquidation_id
    )
    ...
    assert (
        anchor_user_account.is_margin_trading_enabled
        == custom_user_account.is_margin_trading_enabled
    )
...
def cmp_orders(anchor: Order, custom: Order):
    ...
    assert enums_eq(
        anchor.existing_position_direction, custom.existing_position_direction
    )
...
def cmp_perp(anchor: PerpPosition, custom: PerpPosition):
    ...
    assert (
        anchor.last_quote_asset_amount_per_lp == custom.last_quote_asset_amount_per_lp
    )
